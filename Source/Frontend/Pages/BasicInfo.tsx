			...
			if (playerState._id) {
				updatePlayer(playerState._id, playerState)
					.then((data: any) => {
						...
						setStatusMessage(
							`${playerState.place} Updated Successfully!`
						)
						setTimeout(() => {
							setStatusMessage('')
							handleResetPlayerData()
		...
		<View style={styles.content}>
			<View style={styles.inputsContainer}>
				...
				<TextInput
					...
					onChangeText={(text) =>
						setPlayerState({ ...playerState, place: text })
					}
				...
				{playerState.player.map((player, index) => (
					<View style={styles.playerStyle} key={index}>
						<View style={styles.countContainer}>
							...
							>{`Player: ${index + 1}`}</Text>
							<TouchableOpacity
								onPress={() => handleDeletePlayerObject(index)}
							>
								<Icon
									style={styles.doneoutline}
									name={'close'}
